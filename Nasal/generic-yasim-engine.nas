...
var Jet =
{
	...
	new: func(n, running = 0, idle_throttle = 0.01, max_start_n1 = 5.21, start_threshold = 3, spool_time = 4, start_time = 0.02, shutdown_time = 4)
	{
        ...
        m.generator = props.globals.getNode("controls/electric/engine[" ~ n ~ "]/generator", 1);
		m.generator.setBoolValue(0);
        m.generator_switch = props.globals.getNode("controls/electric/generator-switch-main", 1);
		m.generator_switch.setBoolValue(0);
        m.hydrolics_engine_pump = props.globals.getNode("controls/hydraulic/system/engine-pump", 1);
		m.hydrolics_engine_pump.setBoolValue(0);
	...
	update: func
	{
		...
		if (me.cutoff.getBoolValue() or !me.serviceable.getBoolValue() or me.out_of_fuel.getBoolValue())
		{
			...
			else
			{
                ...
                me.hydrolics_engine_pump.setBoolValue(0);
				me.started = 0;
			}
		}
		elsif (me.starter.getBoolValue())
		{
			...
			if (rpm >= me.start_threshold)
			{
				...
				if (rpm >= me.n1.getValue())
				{
					...
					me.started = 1;
                    me.generator.setBoolValue(me.generator_switch.getBoolValue());
                    me.hydrolics_engine_pump.setBoolValue(1);
				}
				else
				{
					me.running.setBoolValue(0);
